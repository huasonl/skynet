...
local cjsondbg = require "cjson"
local vscdebugaux = require "skynet.vscdebugaux"
-- cjsondbg.encode_empty_table_as_array(true)
...
local function recv_request()
    ...
    if header:find("Content-Length: ", 1, true) then
        ...
        if rd then
            ...
            if sreq then
                local ok, req = pcall(cjsondbg.decode, sreq)
                -- skynet.error(sreq)
                if ok then
                    return req
                else
...
local function send_response(cmd, succ, rseq, content)
    ...
    local ok, msg = pcall(cjsondbg.encode, res)
    if ok then
...
skynet.start(function()
    ...
    local ok, bps = pcall(cjsondbg.decode, pbs)
    if ok then
        breakpoints = bps
    end
